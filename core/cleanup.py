...
#
# Попасть сюда обязано ВСЁ, что добавляется в RSYNC_EXCLUDES задним числом, и это не дубль
# списка: `_stale_files` ниже исключённые пути ПРОПУСКАЕТ (они и не должны считаться лишними —
# иначе .env и logs/ попадали бы в список на каждой ноде). Значит новое исключение делает уже
# уехавшие копии НЕВИДИМЫМИ для проверки: не возим больше, а лежащее лежит. `scrips` — ровно
# этот случай, добавлен вместе с исключением.
#
# ИСКЛЮЧЕНИЕ из этого правила — рабочие кадры (`pictures/screenshot*.png`, `pictures/shot*.png`,
# исключены 22-09-2026): они СЮДА НЕ ИДУТ. На ноде это живой рантайм — кадр, который программа
# сняла и через секунду отправит подписчику; снос удалил бы нужный файл. Невидимость для
# проверки «лишнего» здесь и есть цель, как у логов. Подробности — NB в settings/config.py.
_PURGE_NAMES = [".claude", ".vscode", ".idea", ".directory", ".git.backup", "scrips"]
