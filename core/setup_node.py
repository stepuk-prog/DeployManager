...
# операторский доступ флота (ключи + ignoreip) — вендорено в assets/fleet_access
_FLEET_ACCESS_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "assets", "fleet_access")

# remote-скрипт: ставит операторские ключи в root+vova authorized_keys (идемпотентно) и
# добавляет операторские IP в fail2ban ignoreip. __KEYS__/__IPS__ подставляются из ассетов.
_OP_ACCESS_SH = r'''set -e
install -d -m 700 /root/.ssh
[ -f /root/.ssh/authorized_keys ] || touch /root/.ssh/authorized_keys
HASV=0; id vova >/dev/null 2>&1 && HASV=1
if [ $HASV = 1 ]; then
  install -d -m 700 -o vova -g vova /home/vova/.ssh
  [ -f /home/vova/.ssh/authorized_keys ] || touch /home/vova/.ssh/authorized_keys
fi
n=0
while IFS= read -r K; do
  [ -z "$K" ] && continue
  grep -qF "$K" /root/.ssh/authorized_keys || echo "$K" >> /root/.ssh/authorized_keys
  if [ $HASV = 1 ]; then grep -qF "$K" /home/vova/.ssh/authorized_keys || echo "$K" >> /home/vova/.ssh/authorized_keys; fi
  n=$((n+1))
done <<'OPKEYS'
__KEYS__
OPKEYS
chmod 600 /root/.ssh/authorized_keys
if [ $HASV = 1 ]; then chown vova:vova /home/vova/.ssh/authorized_keys; chmod 600 /home/vova/.ssh/authorized_keys; fi
echo "  операторских ключей проверено: $n (root+vova)"
IPS="__IPS__"
if [ -n "$IPS" ] && [ -f /etc/fail2ban/jail.local ]; then
  grep -q '^ignoreip' /etc/fail2ban/jail.local || echo 'ignoreip = 127.0.0.1/8 ::1' >> /etc/fail2ban/jail.local
  for ip in $IPS; do
    grep -qE "^ignoreip.*$ip" /etc/fail2ban/jail.local || sed -i "/^ignoreip/ s| $ip||; /^ignoreip/ s|\$| $ip|" /etc/fail2ban/jail.local
  done
  fail2ban-client reload >/dev/null 2>&1 || systemctl reload fail2ban >/dev/null 2>&1 || true
  echo "  ignoreip дополнен: $IPS"
fi
'''


def _operator_access_data() -> tuple[list[str], list[str]]:
    """Операторские pubkeys + ignoreip-IP из вендоренных assets/fleet_access. Пусто → шаг no-op."""
    def _lines(fname: str) -> list[str]:
        path = os.path.join(_FLEET_ACCESS_DIR, fname)
        ...
        for raw in open(path, encoding="utf-8"):
            ...
            if s and not s.startswith("#"):
                out.append(s)
        return out
    return _lines("operator_keys.pub"), _lines("operator_ignoreip.txt")
...
async def run_setup_node(db: Database, ssh: SshClient, *, dry_run: bool = False) -> None:
    ...
    # ── операторский доступ (для любого типа ноды) ──
    async def _act_operator_access() -> bool:
        keys, ips = _operator_access_data()
        if not keys and not ips:
            print("  (нет assets/fleet_access/* — операторский доступ пропущен)")
            return True
        script = _OP_ACCESS_SH.replace("__KEYS__", "\n".join(keys)).replace("__IPS__", " ".join(ips))
        b64 = base64.b64encode(script.encode("utf-8")).decode("ascii")
        r = await ssh.run(ip, f"echo {b64} | base64 -d | bash", timeout=90, user=_PRIV)
        if r.stdout:
            print(r.stdout)
        if not r.ok:
            print(f"🛑 {r.stderr or r.stdout or ('exit ' + str(r.exit_status))}")
        return r.ok
    ...
    # операторский доступ (ключи флота + ignoreip) — для ЛЮБОГО типа ноды
    res = await _do_step(
        "operator_access", "Операторский доступ (ключи флота + ignoreip)",
        "Ставит стандартные операторские SSH-ключи флота (cluster_root@PRIMARY, root@primary-pq-f) "
        "в root+vova authorized_keys и добавляет операторские IP в fail2ban ignoreip. Без него новая "
        "нода не пускает коллег/автоматизацию по их ключам (провижн кладёт только твой ключ). "
        "Список — assets/fleet_access/operator_keys.pub + operator_ignoreip.txt.",
        _act_operator_access)
    if res != "ok":
        return
