...
# КОНТРАКТ потока ожидания кода: внешний потолок = CODE_WAIT_SECONDS + CODE_STEP_SLACK, то есть
# за дедлайн разрешено выйти ДВУМ операциям подряд. Не одной: `wait_for_code` намеренно
# дочитывает письмо, чей uid нашёлся последним поиском, — а поиск сам мог стартовать за
# мгновение до дедлайна и вернуться медленно, но УСПЕШНО (сокет-таймаут не сработал, ответ
# пришёл). Тогда за окно выходят ОБЕ: поиск и чтение-развязка. Держат контракт три вещи —
# укороченный сокет-таймаут реконнекта (три операции укладываются в одну, см. ниже), проверки
# дедлайна ВНУТРИ опроса (`wait_for_code` / `code_uids`) и запас CODE_STEP_SLACK.
# Без проверок дедлайна опрос, начатый на 119-й секунде, успевал сделать noop + поиск НА
# КАЖДОГО отправителя + чтение письма — четыре сокет-таймаута подряд, ~80с. Ломалось это ровно
# так же, как обрыв до 0.6.3:
...
# Сам запас: ДВЕ сокет-операции, а не одна (почему — в контракте выше). Считаем их по
# IMAP_OP_TIMEOUT, а не по IMAP_SOCKET_TIMEOUT: тот же зазор поверх сокета, что и у прочих
# операций в потоке. В потолок потока и в бюджет входа это число попадает ЧЕРЕЗ
# `code_step_timeout()` — одним выражением на оба места, чтобы им нечем было разъехаться.
CODE_STEP_SLACK = 2 * IMAP_OP_TIMEOUT
...
# `CODE_WAIT_SECONDS + CODE_STEP_SLACK` (см. КОНТРАКТ выше), и реконнект обязан помещаться в
# ОДНУ операцию этого запаса. А он — ТРИ операции (connect, login, select): с обычными 20с он
# выбивал бы из контракта, обрыв на 119-й секунде тянул хвост до ~182с при тогдашнем потолке
# 150, после чего `wait_for` снимал ожидание, вход объявлялся
...
# Сюда НЕ входит `safe_logout` мёртвого соединения, который делается перед реконнектом: он идёт
# по СТАРОМУ сокету с его собственным IMAP_SOCKET_TIMEOUT, и в худшем случае это ещё столько же.
# Его покрывает внешний запас (CODE_STEP_SLACK), и проверка budget_contract это стережёт —
# поднимете IMAP_SOCKET_TIMEOUT, не тронув запас, тест упадёт раньше прода.
...
def code_step_timeout() -> float:
    """Внешний потолок ПОТОКА, который ждёт код с почты: окно ожидания плюс запас на хвост
    (см. КОНТРАКТ у CODE_STEP_SLACK).

    Функция, а не константа, и зовётся ровно из двух мест — `asyncio.wait_for` вокруг
    `wait_for_code` в `inline_login` и слагаемое «код с почты» в `login_budget`. Оба обязаны
    называть ОДНО число: потолок потока, который бюджет не покрывает, — это вход, снятый
    снаружи раньше, чем он успел отработать сам. Раньше выражение стояло литералом в обоих
    местах, то есть ждало ровно той же судьбы, от которой заведён сам `login_budget`."""
    return CODE_WAIT_SECONDS + CODE_STEP_SLACK
...
async def imap_thread(fn, *args, timeout: float = IMAP_OP_TIMEOUT):
    ...
    async-флоу навсегда. asyncio.to_thread сам не отменяем, но await вернётся по таймауту →
    флоу не залипает, а поток-сирота доживает сам (см. про его соединение в inline_login)."""
...
async def inline_login(page, context, *, mail: str, app_pass: str, sel: dict,
    ...
    try:
        ...
        # imap_thread упёрся в потолок wait_for: поток-сирота, возможно, ещё держит imap. Звать
        # safe_logout на ТОМ ЖЕ объекте из finally нельзя — параллельная работа двух потоков на
        # сокете imaplib (он не thread-safe) даёт гонку, а она дороже незакрытого соединения.
        imap_timed_out = True
        ...
        # logout пропускаем, если была гонка-опасная отмена по таймауту (см. выше).
        #
        # ЧТО СТАНЕТ С ТЕМ СОЕДИНЕНИЕМ (формулировка уточнена 21-09-2026, прежняя обещала
        # больше, чем есть). Сокет-таймаут стоит на ОПЕРАЦИИ, а не на простое: соединение,
        # которое никто не читает, сам по себе он не закроет. Закрывает его сборка мусора —
        # как только объект перестаёт быть достижимым, CPython по счётчику ссылок дёргает
        # деструктор сокета и fd уходит; в худшем случае (ссылка задержалась) оно доживает до
        # idle-таймаута Gmail, это порядка получаса. Накапливаться нечему: за один вход такое
        # соединение одно (старое, если был реконнект, погашено прямо в его ветке), а лимит
        # Gmail — 15 параллельных сессий.
        #
        # ⚠️ Есть и третий путь, микроскопический: поток отдал код ровно в момент срабатывания
        # wait_for. Тогда `delivered=True` гасит уборку внутри wait_for_code, а здесь мы уже на
        # ветке таймаута и тоже не гасим — то есть не гасит НИКТО. Судьба та же, что описана
        # выше; заводить ради этого контейнер для передачи соединения наружу дороже, чем
        # оставить сокет сборщику.
