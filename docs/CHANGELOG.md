...
### Added
- **Скрипт флота «Утренний проход» (`morning_sweep`, 🌅, scope=local, read-only).** Свод ночи
  по клиентским нодам: `error`/`critical`/`cookies`/`warning` всех программ под
  `/home/vova/**/logs/**` за окно `--since` (по умолчанию «yesterday 18:00», всё, что понимает
  `date -d`), нормализованные по типам — счёт, список программ, для ошибок последняя строка
  целиком; `--failed`-юниты, выходы по сбою/OOM из journal, память и load. Сбор по нодам
  параллельный. Из CLI: `main.py --action morning_sweep --yes`.
  Проход 21–23-09 жил в `/tmp` и скретчпадах сессий и дважды пропал с перезагрузкой машины —
  теперь он вендорен в `assets/fleet_scripts/morning-sweep.sh`, как остальные скрипты флота.
  События диспетчера (БД) в него не входят — это отдельный запрос.

## 2026-09-09

### Added
- **Суб-инструмент «Юзерботы (сессии)» разделён на ДВЕ ГРУППЫ по базам.** Появился второй
  парк юзерботов — аккаунты ИИ-персонажей форума (проект ForumPersonas), и живёт он в
  ДРУГОЙ базе: `forum.person.account` против флотовой `Program.telegram.telegram`.
  Сджойнить их нечем, а запись сессии не в ту таблицу молча ломает и аккаунт, и программу,
  которая им работает — потому разделение на входе в инструмент, а не фильтром в списке.
  - `tools/sessions/__init__.py` — выбор группы. Старые действия `list/recover/create`
    по-прежнему ведут во флот (обратная совместимость CLI/GUI), `person:<ветка>` — сразу
    к персонажам.
  - `tools/sessions/fleet.py` — флотовые ветки (вынесены из `__init__.py` без изменений).
  - `tools/sessions/personas.py` — ветки персонажей: аккаунты · проверить сессии (getMe) ·
    обновить/создать сессию · приватность · персонажи с чертами.
  - `tools/sessions/person_tg.py` — Telegram-операции готовой сессией (без логина):
    `check_alive` и `privacy` с нормой приватности персонажа (номер — никому, звонки —
    никому, добавление в чаты и время захода — контакты).
  - `database/person.py` — `PersonDatabase(Database)`: пул на БД `forum`, запросы к
    `person.account` / `person.persona`.
  - Конфиг: `PG_DB_FORUM`, `PERSON_TG_EXEC`, `PERSON_TG_WORKDIR`.

### Changed
- `database/db.py`: имя БД параметризовано — `Database(min_size, max_size, database=…)`.
  Раньше было намертво `config.PG_DATABASE`. Наследник ходит в другую базу того же
  кластера, не копируя машинерию пула, ретраев и контракт ошибок.
- Подписи меню CLI (`[6]`) и `--help`: «флот (БД Program) и персонажи форума (БД forum)».

## 2026-09-03

### Fixed
- **`scope: local` скрипты не могли зайти НИ на один узел.** `_gen_nodes_sh` собирал
  `SSH_OPTS` без ключа, а `ssh_node` ходит по IP — `IdentityFile` из `~/.ssh/config`
  привязан к Host-АЛИАСАМ и по IP не матчится. До ротации ролей 28-08 это не всплывало:
  ключ звался `id_rsa`, и ssh подхватывал его сам как дефолтный. После переименования
  в `id_vlad_2026` неявный подхват исчез → `Permission denied (publickey)` на всех
  13 узлах. Теперь ключ берётся из собственного `.env` DM (`PRIV_KEY`, иначе `SSH_KEY`)
  вместе с `PRIV_USER` и `SSH_CONNECT_TIMEOUT` — путь оператора в коде не появляется.
  Кроме «Аудита кластера» это чинит **`Whitelist IP` и `Смена IP узла`**: они
  отрабатывали «успешно», не сделав на флоте ничего.
- **`audit-cluster.sh`, секция 3: недоступный SSH выглядел как авария HAProxy.**
  `2>/dev/null` глушил stderr ssh, и три разные причины схлопывались в одно
  «лидер не виден / нет сокета». Диагнозы разделены: `SSH недоступен` (с кодом и
  последней строкой ошибки) / `haproxy не запущен` / `нет сокета` / `нет socat` /
  `HAProxy жив, но лидера нет`. При успехе печатается имя backend-сервера лидера.

### Changed
- **`audit-cluster.sh`, секция 3: лидер печатается по имени из БД, а не по метке haproxy.**
  На `node1…node6` метки `server` исторические — `node4` = cluster1, `node3` = cluster3
  (при живом клиенте node-3 на другом IP). Теперь резолвим IP→`vocabulary.nodes`, а метку
  показываем ТОЛЬКО когда она расходится, чтобы расхождение было видно. Заодно колонки
  csv ищутся по заголовку, а не по номеру: захардкоженный `$18` поехал бы с версией haproxy.
- **`audit-cluster.sh`, секция 4: ServerManager2 больше не ложное срабатывание.**
  Правило «ExecStart под `/home/vova` = прикладная программа» верное — GD2/WD2/CD2 живут
  в `/opt`. SM2 единственный в `/opt` не переехал, автозапуск ему положен. Вынесен
  отдельной строкой «инфра-боты в автозапуске (норма)», а не спрятан: иначе забудем,
  что он ещё в `/home/vova`. Обход всего флота дал ровно одно это срабатывание.

## 2026-09-02

### Changed
- **Реестр узлов для `scope: local` — весь `vocabulary.nodes`, без фильтра `is_online`.**
  Новый `db.get_fleet_nodes()`; на него переведены `_gen_nodes_sh` и шаг `whitelist`
  мастера. Временно недоступный узел не должен выпадать из реестра: иначе остальные
  перестанут его пускать и сетевой сбой превратится в потерю доступа. Для деплоя и
  операций НА узлах по-прежнему `get_online_nodes()`.

## 2026-08-30

### Changed
- **`SSH_CONNECT_TIMEOUT` 10 → 25с.** На node-2 моргает линк, и десяти секунд не хватало на
  установление соединения с живым узлом — дашборд рисовал ⛔ там, где нода была в порядке.
  Ложный ⛔ дороже лишних 15с ожидания: он уводит ноду из синхронизации ровно так же, как
  реальный обрыв.

### Fixed
- **Таймаут пула PostgreSQL больше не зависит от SSH.** `database/db.py` передавал
  `asyncpg.create_pool(timeout=...)` значение `SSH_CONNECT_TIMEOUT` — совпадение по смыслу
  случайное, и подъём SSH-таймаута молча утащил бы за собой ожидание коннекта к БД.
  Заведён отдельный `PG_CONNECT_TIMEOUT` (по умолчанию прежние 10с).

## 2026-08-15

### Fixed
- **Нода без читаемого `VERSION` молча выпадала из синхронизации.** `update` берёт ровно тот
  список, что дашборд положил в `stale`, а дашборд на любую неудачу чтения манифеста печатал
  `🔌 нет VERSION` и делал `continue` — нода в `stale` не попадала. Отсутствие версии
  трактовалось как «трогать не надо», хотя означает ровно обратное: версия НЕизвестна.
  Инцидент 2026-08-15: BinoOptions уехал на 5 нод из 7 (NODE-3/NODE-4 остались со старым
  кодом), причём именно на NODE-4 в этот момент работал бот, и обновление до него не дошло.
  Занижение было невидимым: в подтверждении «Обновить код на N нодах?» N уже меньше нужного,
  а `🔌`-строки теряются выше по выводу дашборда.
- **`_read_manifest` различает «файла нет» и «нода не ответила».** Раньше `read_file` отдавал
  `None` в обоих случаях. Теперь читаем через `ssh.run` и смотрим `exit_status`: `255` — это
  транспорт (таймаут/обрыв, код ставит `SshClient.run`), прочий ненулевой — `cat` не нашёл
  файл либо нет прав. Битый/пустой JSON тоже считается «версии нет».
- **Обе категории идут в синхронизацию** с пометкой `unknown=True` и `lag="версия неизвестна
  (…)"`. Транспортный сбой включаем осознанно: честная ошибка деплоя лучше тихого пропуска.
  В `update` они выводятся отдельной строкой `⚠️ Версия не выяснена (беру в синхронизацию)` —
  рядом с уже существующим предупреждением про offline-ноды, до вопроса о подтверждении.
- Тесты: 4 новых на `_read_manifest` (валидный манифест / нет файла / нода не ответила /
  битый JSON), всего 55.

## 2026-07-29

### Fixed
- **`.env` компонента под root ломал и запись, и чтение конфига.** На 12 старых нодах
  `/opt/DispatcherCtl/.env` остался от ручной установки (`root:root 600`) → `dispatcherctl`
  под `vova` падал `PermissionError`, а `sync-env` не мог перезаписать файл (пишем от `vova`).
  На свежих нодах (VIDEO-3, мастер setup-node) владелец уже верный — `vova:vova`.
  `_write_env` теперь на провале записи один раз нормализует владельца под root
  (`chown vova:vova` + `chmod 600`) и повторяет запись. Флот приведён к `vova:vova 600`.
