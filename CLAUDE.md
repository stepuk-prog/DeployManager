...
## Гайдлайны/гочи
- **Прод!** Деструктивное в БД/на нодах — только с подтверждением; деплой/uninstall логировать в журнал.
- Деплой — только **первичный** и **добавление серверов**. Обновление уже развёрнутого — **отдельная ветка (ещё не сделана)**.
...
- Цвет лога: ✅/up-to-date зелёный; ⛔/❌/‼️/🛑/ошибка/⚠️⚠️ красный; ⚠️/stale/DIRTY янтарный.

## Состояние (на момент записи)
- Ветка **`feature/gui`** (всё запушено). Готово: первичный деплой, дашборд, state-check, leader-aware, watchdog, деинсталляция-проект, журнал, GUI (Flet), цветной лог, ui.select/checkbox.
- Обкатано вживую: **деинсталляция — успех**. Деплой до конца ещё не довёл (раньше падал на install_services из-за sudo → теперь `PRIV_USER=root`).
- **Дальше:** довести первичный деплой вживую (на preflight «перезаписать» остатки) → проверить `VERSION`/привязки/журнал → слить `feature/gui` → `master`.
