...
пустой `PRIV_KEY` = общий `SSH_KEY`).

Этим же ключом ходят `scope: local` скрипты флота: DM подставляет его в
сгенерённый `_nodes.sh`, который обходит узлы **по IP**. Полагаться на
`~/.ssh/config` там нельзя — `IdentityFile` в нём привязан к Host-алиасам,
а по IP они не матчатся.
